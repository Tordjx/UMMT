...
def cycle_consistent_forward(model_A,model_B,text_input,target, image_input = None, image_bool = False) : 
    ...
    if np.random.rand() < 1/2 : #DO NOT TEACHER FORCE
        decoder_input = torch.cat((torch.ones(text_input.shape[0], 1, dtype = torch.int).fill_(model_B.begin_id),torch.ones(text_input.shape[0] ,text_input.shape[1]-1,dtype = torch.int).fill_(model_B.padding_id)),dim =1)
